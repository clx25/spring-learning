...
public class MapperScannerConfigurer
  ...
  public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
    ...
    /*
    创建一个扫描器，这个扫描器类似spring的ComponentScanAnnotationParser，用来解析
    @MapperScan中配置的包
     */

    ClassPathMapperScanner scanner = new ClassPathMapperScanner(registry);
    //配置扫描器属性，这里的部分属性已经在上方processPropertyPlaceHolders()中被初始化
    ...
    /*
    设置扫描器的过滤器，这个过滤器会判断@MapperScan扫描出来的class文件
    是否includeFilters这里默认为只返回true
     */
    scanner.registerFilters();
    //这个scan调用的是父类(ClassPathBeanDefinitionScanner)的方法，
    // 而父类方scan法中的doScan调用的是本类（ClassPathMapperScanner）的doScan
    scanner.scan(
  ...
  private void processPropertyPlaceHolders() {
    ...
    if (!prcs.isEmpty() && applicationContext instanceof ConfigurableApplicationContext) {
      //这里的applicationContext和beanName是通过BeanNameAware和ApplicationContextAware注入
