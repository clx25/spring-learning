...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  public Set<BeanDefinitionHolder> doScan(String... basePackages) {
    //调用spring的doScan，扫描出class并转为beanDefinition
    ...
    if (beanDefinitions.isEmpty()) {
      ...
      //对beanDefinition进行修改，最主要的就是把自动装配模式改为byType
...
/*
这个方法对原本是mapper的BeanDefinition进行了修改，
改成一个MapperFactoryBean的BeanDefinition，但是beanName仍然为原来的spring生成的beanName
这个MapperFactoryBean实现了FactoryBean，能从getObject中能返回mapper的代理对象
所以使用mapper的beanName能获取代理对象，使用&beanName能获取到这个MapperFactoryBean对象
由于设置了这个BeanDefinition的自动装配模式为byType，所以spring会寻找MapperFactoryBean中的set方法
并注入相关数据，MapperFactoryBean父类中有setSqlSessionFactory方法，spring会注入sqlSessionFactory
并在该set方法中初始化了sqlSessionTemplate，所以需要在配置类中配置SqlSessionFactory
 */
  private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
	...
	//添加按泛型匹配的类型参数值，就是使用带泛型的构造器
      ...
      //设置fanctoryBean中getObjectType返回的类型
