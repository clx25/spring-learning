...
public class MapperFactoryBean<T> extends SqlSessionDaoSupport implements FactoryBean<T> {
  ...
  //由于ClassPathMapperScanner中对beanDefinition的设置，spring会选择这个构造器创建对象
  //注入的类型为mapper的接口类型
   ...
   * {@inheritDoc}
   */
  //父类实现了InitializingBean，spring在调用实现方法时调用到该方法
  @Override
  protected void checkDaoConfig() {
    ...
    if (this.addToConfig && !configuration.hasMapper(this.mapperInterface)) {
      try {
        //这里完成了configuration属性的设置，如sql的解析，注解，xml，配置的解析，缓存，sql方言的设置
